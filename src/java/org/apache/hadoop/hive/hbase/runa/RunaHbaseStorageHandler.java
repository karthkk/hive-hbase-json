 ...
 */
public class RunaHbaseStorageHandler implements HiveStorageHandler,
		HiveMetaHook {
	...
	private String getHBaseTableName(Table tbl) {
		...
		String tableName = tbl.getParameters().get(
				RunaHbaseSerDe.HBASE_TABLE_NAME);
		if (tableName == null) {
			tableName = tbl.getSd().getSerdeInfo().getParameters()
					.get(RunaHbaseSerDe.HBASE_TABLE_NAME);
		}
		if (tableName == null) {
	...
	public void preCreateTable(Table tbl) throws MetaException {
		...
		try {
			...
			Map<String, String> serdeParam = tbl.getSd().getSerdeInfo()
					.getParameters();
			String hbaseColumnStr = serdeParam
					.get(RunaHbaseSerDe.HBASE_COL_MAPPING);
			if (hbaseColumnStr == null) {
				throw new MetaException(
						"No hbase.columns.mapping defined in Serde.");
			}
			List<String> hbaseColumns = RunaHbaseSerDe
					.parseColumnMapping(hbaseColumnStr);
			int iKeyFirst = hbaseColumns.indexOf(RunaHbaseSerDe.HBASE_KEY_COL);
			int iKeyLast = hbaseColumns
					.lastIndexOf(RunaHbaseSerDe.HBASE_KEY_COL);
			if (iKeyFirst != iKeyLast) {
				throw new MetaException(
						"Multiple key columns defined in hbase.columns.mapping.");
			}
			for (String hbaseColumn : hbaseColumns) {
				if (RunaHbaseSerDe.isSpecialColumn(hbaseColumn)) {
					continue;
				}
				int idx = hbaseColumn.indexOf(":");
				if (idx < 0) {
					throw new MetaException(hbaseColumn
							+ " is not a qualified hbase column.");
				}
			...
			if (!getHBaseAdmin().tableExists(tblName)) {
				...
				tblDesc = getHBaseAdmin().getTableDescriptor(
						Bytes.toBytes(tblName));
				for (String cf : columnFamilies) {
	...
	public void configureTableJobProperties(TableDesc tableDesc,
		...
		jobProperties.put(RunaHbaseSerDe.HBASE_COL_MAPPING,
				tableProperties.getProperty(RunaHbaseSerDe.HBASE_COL_MAPPING));

		String tableName = tableProperties
				.getProperty(RunaHbaseSerDe.HBASE_TABLE_NAME);
		if (tableName == null) {
			tableName = tableProperties.getProperty(Constants.META_TABLE_NAME);
		}
		jobProperties.put(RunaHbaseSerDe.HBASE_TABLE_NAME, tableName);
